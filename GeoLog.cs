...
namespace GeoLog
{
    public class GeoLog : Mod
    {
        ...
        private List<KeyValuePair<string, string>> _doneGos = new List<KeyValuePair<string, string>>();
        ...
        private void CheckHealthManager(HealthManager self)
        {
            try
            {
                if (_doneGos.Exists(x => x.Key == self.gameObject.scene.path && x.Value == self.gameObject.GetGoPath())) return;
                ...
                Log($"{self.gameObject.scene.path}, {_pdToNameMap[name]}, {sg}, {mg}, {lg}, {sg + (mg * 5) + (lg * 25)}, {Mathf.CeilToInt(sg * 1.2f) + (Mathf.CeilToInt(mg * 1.2f) * 5) + (Mathf.CeilToInt(lg * 1.2f) * 25)}");
                _doneGos.Add(new KeyValuePair<string, string>(self.gameObject.scene.path, self.gameObject.GetGoPath()));
            }
            catch (Exception )
            {
            }
